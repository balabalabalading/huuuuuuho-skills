...
description: >
  生成微信公众号「高效人生指北」的静态图文长文。当用户想把工作流探索、AI 工具测评、产品体验、个人实践、生活感悟等素材整理成公众号文章时使用。包括意图确认、大纲、文章撰写、事实核查、标题推荐、视觉脚本、3.35:1 单张组合封面、正文插图生产、PicGo 图床上传、行文与视觉自检。公众号封面使用 guizang-social-card-skill；正文解释性插图在询问环节由作者选择 guizang-social-card-skill 或 guizang-material-illustration。即使用户只是说「帮我写篇文章」「整理成推文」「发公众号」，也应当触发此技能。不生成 Live Photo、视频或其他平台素材。
---
...
先检查当前环境是否能够读取 `guizang-social-card-skill` 和 `guizang-material-illustration`，并检查本机 PicGo Server 是否可访问。两个 Skill 都是完整静态视觉工作流的推荐前置条件，但不得自动安装、不得声明为强制依赖。PicGo 不可用时仍保留本地成品，Step 11 明确标记「图床上传待完成」，不得把本地图片引用当成可直接复制到公众号的最终状态。
...
- **授权方式**：本次对选定归藏 Skill 的首次渲染授权，同时授权后续自动执行静态图片检查、必要修复，并在 Step 10 通过作者已配置的 PicGo Server 上传最终图片；无需在检查或上传前重复询问。
...
静态检查通过后，运行 `scripts/upload-images-to-picgo.mjs`，将一张组合封面和全部最终正文图片批量上传到本机 PicGo Server。脚本保留本地可编辑源文件，只创建临时上传副本，并以「文章标题、素材角色、时间戳」生成唯一图床文件名。用 PicGo 返回的 HTTPS 地址更新文章：front matter 的 `cover` 指向组合封面，正文使用 `![中文说明|500](https://...)`。同时在 `SOURCES.md` 记录本地文件与远程地址的映射。

上传后逐个验证远程地址返回 `2xx` 且内容类型为图片。不得读取、输出或写入腾讯云 SecretId、SecretKey；上传配置完全交给作者已经配置的 PicGo。PicGo Server 默认地址为 `http://127.0.0.1:36677/upload`，可用 `--endpoint` 或 `PICGO_SERVER_URL` 覆盖。服务启用鉴权时只从 `PICGO_SERVER_SECRET` 环境变量取得 shared secret，不读取 PicGo 图床配置文件。
...
- PicGo 可用时，组合封面和正文图片均已上传，文章只引用通过可访问性检查的 HTTPS 图片地址；PicGo 不可用时已明确列出上传待完成项和恢复命令。
...
- [ ] **图床发布**：PicGo 可用时，组合封面和正文图片已上传并通过 HTTPS 可访问性检查，文章中不存在最终交付所依赖的本地图片路径；PicGo 不可用时已标记为待完成
