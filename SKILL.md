...
如果素材不足以支撑一篇 4000-8000 字的文章，主动向用户确认需要补充哪些内容，而不是自行编造。这一点至关重要——作者的可信度建立在真实性之上，编造细节或数据是不可接受的。
...
标题要能够激发人的好奇心，切入点应当是普通人的视角，但不要哗众取宠。

## 行文规范

参考 references/行文风格指南.md（少数派创作手册风格指南），作为行文排版和标点符号的权威参考。仅在需要查阅具体排版规则时阅读该文件。
...
- [ ] **front matter**：符合 templates/templater_MPContent 模板格式
...
- references/行文风格指南.md — 少数派创作手册风格指南，作为行文排版和标点符号的权威参考。仅在需要查阅具体排版规则时阅读，不需要每次全量加载。
- references/范文风格分析.md — 从作者历史文章中提炼的风格特征和写作模式，生成文章前建议先阅读以校准语感。
- templater_MPContent.md — 文章 front matter 模板
